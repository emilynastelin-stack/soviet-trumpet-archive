...
(function(){
  ...
  // No select/clear controls for Type group â only populate checkboxes
      list.forEach((val,i)=>{
  ...
  function renderPage(page){
    ...
    Array.from(container.querySelectorAll('.details-link')).forEach(link => {
      link.addEventListener('click', (e) => {
        ...
        // Note: do NOT call loadResults here â keeping details open requires avoiding a full re-render
      });
    });
  ...
  async function populateComposerBox(name, row){
    ...
    const looksLikeCompDet = (sampleObj) => {
      ...
      const hasComposer = keys.some(k => k.includes('composer') || k.includes('ÐºÐ¾Ð¼Ð¿Ð¾Ð·Ð¸ÑÐ¾Ñ'));
    ...
    const composerKey = sampleKeys.find(k => normalizeKey(k).includes('composer') || normalizeKey(k).includes('ÐºÐ¾Ð¼Ð¿Ð¾Ð·Ð¸ÑÐ¾Ñ'));
  ...
  (async function init(){
    try{
      const resultsList = document.getElementById('results-list'); if (resultsList) resultsList.innerHTML = '<div class="result-item"><em>Loading resultsâ¦</em></div>';
  const fc = document.getElementById('filter-country'); if (fc) fc.innerHTML = '<div style="color:#6b7280">Loadingâ¦</div>';
  const fd = document.getElementById('filter-decade'); if (fd) fd.innerHTML = '<div style="color:#6b7280">Loadingâ¦</div>';
  const ft = document.getElementById('filter-type'); if (ft) ft.innerHTML = '<div style="color:#6b7280">Loadingâ¦</div>';
  const fg = document.getElementById('filter-gender'); if (fg) fg.innerHTML = '<div style="color:#6b7280">Loadingâ¦</div>';
