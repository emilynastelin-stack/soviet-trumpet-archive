...
(function(){
  ...
  async function populateComposerBox(name, row){
    ...
    const looksLikeComposerDetails = (sampleObj) => {
  ...
  if (Array.isArray(jr) && jr.length){ const sample = jr[0]; if (looksLikeComposerDetails(sample)){ rows = jr; sourceSheet = sname; console.debug('COMPOSER_DEBUG selected sheet (api):', sname, 'headers:', Object.keys(sample)); break; } else { console.debug('COMPOSER_DEBUG rejected sheet (api):', sname, 'headers:', Object.keys(sample)); } }
      ...
      for (const sname of candidates){
        try{
          ...
          if (g && g.rows && g.rows.length){
            ...
            if (looksLikeComposerDetails(sampleObj)){
